...
ZeroDev leverages [batching](/smart-accounts/batch-transactions) and [delegatecall](/smart-accounts/delegatecall) internally to ensure that even complex routes are executed in one atomic UserOp, providing the user with low latency, low gas cost, and high safety.

## Supported Tokens

See the full lists of supported base tokens and DeFi tokens:

- [Base tokens](/smart-accounts/chain-abstraction/supported-base-tokens)
- [DeFi tokens (protocols)](/smart-accounts/chain-abstraction/supported-defi-tokens)
...
- `kernelClient` is the [account client](/onboarding/create-a-smart-account#create-an-account-client) object.
