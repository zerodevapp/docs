...
# Sending Transactions [In ERC-4337, a transaction is known as a "UserOp"—a regular transaction with extra AA-specific information, such as whether it should be sponsored.]
...
<Cards>
  <Card icon="transaction" title="Viem API">
    Send regular transactions through the Viem API, which ZeroDev then "translates" into UserOps. More interoperable with existing libraries like Viem that deal only with transactions and not UserOps.
  </Card>
  <Card icon="send" title="Raw UserOps">
    Send raw UserOps directly for the highest degree of flexibility.
  </Card>
</Cards>

We will now describe both approaches.  We assume that you have already [created a Kernel account](/onboarding/create-a-smart-account).
