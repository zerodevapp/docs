...
We assume that you have [created a Kernel account](/onboarding/create-a-smart-account#create-a-kernel-account).  If you only have a `kernelClient`, the Kernel account can be accessed as `kernelClient.account`.
...
If you want to use the session key on the same node, you can already [construct a Kernel client from this Kernel account](/onboarding/create-a-smart-account#create-an-account-client) and start using it.  If you want to share the session key with an agent or store it for use later, you can serialize the session key:
...
Now you can [create a Kernel client](/onboarding/create-a-smart-account#create-an-account-client) using the `sessionKeyAccount` as you normally would.
...
:::details[Does creating session keys cost gas?]

No.  Creating a session key entails simply signing a message, which is done off-chain and doesn't involve any gas cost.

:::

:::details[Is it possible to use session keys with a not-yet-deployed account?]
