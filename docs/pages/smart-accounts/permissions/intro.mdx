# Permissions (Session Keys) [With Kernel, you can assign different permissions to different keys.]

Some of these keys might be owned by the owner(s) of the smart account, and some might be short-lived keys that you share with others to delegate transactions.  The latter are also commonly known as "session keys."
