# Using Plugins [ZeroDev is built on Kernel, a modular smart account that can be extended with plugins (sometimes also called modules).]
...
When you set up a [Kernel account object](/onboarding/create-a-smart-account#create-a-kernel-account) with the SDK, you must specify a sudo validator, a regular validator, or both.  Let's walk through the three cases:
...
Learn more about plugins such as [passkeys](/onboarding/passkeys/overview) and [permissions](/smart-accounts/permissions/intro).
