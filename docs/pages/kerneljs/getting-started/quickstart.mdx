# Quickstart with Kernel.js

:::info
Impatient?  Check out [the complete example here](https://github.com/zerodevapp/kernel.js-examples/tree/main/quickstart/completed.ts).
:::

In this tutorial, we will mint an NFT without paying gas.  We assume that you have a high-level understandng of AA concepts such as bundlers, paymasters, and UserOp; if not, read [the introduction](/kerneljs/getting-started/intro) first.

## Create a ZeroDev Project

Kernel.js works with any bundlers and paymasters.  For this tutorial, we will use ZeroDev's [meta AA infra](/meta-infra/intro).
...
Clone the [Kernel.js examples repo](https://github.com/zerodevapp/kernel.js-examples).  Then, inside the directory, install all dependencies:
...
```bash
npx ts-node quickstart/completed.ts
```

Now open the [`quickstart/template.ts`](https://github.com/zerodevapp/kernel.js-examples/blob/main/quickstart/template.ts) file in your editor.  This will be the template where you will write your code.  You can always refer to [`quickstart/completed.ts`](https://github.com/zerodevapp/kernel.js-examples/blob/main/quickstart/completed.ts) to see the completed tutorial code.

### Creating a Kernel client

Before you can send UserOps, you need to construct an "account client."  An account client is an object that interacts with a bundler on behalf of a specific account.  In the case of Kernel.js, all account clients are of the type `KernelAccountClient`.

There are several types of Kernel account clients, each for a different type of [validation methods](/kerneljs/plugins/intro).  For this tutorial, we will be using the most common validation method -- ECDSA, which is the same algorithm used by normal Ethereum accounts (EOAs).
...
Run this script with `npx ts-node quickstart/template.ts` and confirm that it prints an address.
...
Run the script again with `npx ts-node quickstart/template.ts` and confirm that it prints the UserOp hash.  At this point, you can go to a UserOp explorer such as [JiffyScan](https://jiffyscan.xyz/) and find your UserOp with the hash!
