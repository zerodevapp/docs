...
:::info
Impatient?  Check out [complete examples here](https://github.com/zerodevapp/kernel.js-examples/blob/main/session-keys).
:::
...
:::info
Check out [the code example](https://github.com/zerodevapp/kernel.js-examples/blob/main/session-keys/owner-created.ts) for the owner-created pattern.
:::

We assume that you have [created a Kernel account](/kerneljs/core-api/create-account#create-a-kernel-account).  If you only have a `kernelClient`, the Kernel account can be accessed as `kernelClient.account`.
...
Now, construct a Kernel account using your default validator as the `defaultValidator` and the session key validator as the `validator`:
...
In this example, the default validator is `ecdsaValidator`.

If you want to use the session key on the same node, you can already [construct a Kernel client from this Kernel account](/kerneljs/core-api/create-account#create-an-account-client) and start using it.  If you want to share the session key with an agent or store it for use later, you can serialize the session key:

```typescript
import { serializeSessionKeyAccount } from "@kerneljs/session-key"
...
Now you can [create a Kernel client](/kerneljs/core-api/create-account#create-an-account-client) using the `sessionKeyAccount` as you normally would.

### Agent-created

:::info
Check out [the code example](https://github.com/zerodevapp/kernel.js-examples/blob/main/session-keys/agent-created.ts) for the agent-created pattern.
:::
...
Now, the owner can construct a Kernel account using the default validator as `defaultValidator` and the session key validator as the `validator`:
...
If set to `address(1)` (you can use `oneAddress` from `@kerneljs/session-key`), the session key will only work with a paymaster, but it can be any paymaster.
...
```typescript
import { ParamOperator } from "@kerneljs/session-key"
...
- `target`: the target contract to call.  If this is `zeroAddress`, then the target can be any contract as long as the ABI matches.
