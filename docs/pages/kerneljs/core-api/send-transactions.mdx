...
:::info
Impatient?  Check out [complete examples here](https://github.com/zerodevapp/kernel.js-examples/tree/main/send-transactions).
:::
...
- Sending raw UserOps
- Sending regular transactions through the Viem API, which Kernel.js then "translates" into UserOps
...
We will now describe both approaches.  We assume that you have already [created a Kernel account](/kerneljs/core-api/create-account).
...
- `entryPoint` defaults to the current canonical EntryPoint.
...
Note that if you had constructed your `kernelClient` with a [preset](/kerneljs/presets/intro), you may not have access to a bundler client.  Instead, you can extend your `kernelClient` with bundler actions:
