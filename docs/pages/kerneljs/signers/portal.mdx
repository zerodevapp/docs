# Use Portal with kernel.js

[Portal](https://www.portalhq.io/) is an embedded wallet provider that enables users to generate wallets within your application using Social Logins, Email OTP, or Webauthn.

## Set up

To use Portal with ZeroDev, you have two options: Portal's native integration using ZeroDev, or a custom integration using Portal's Externally Owned Account (EOA) as a signer for kernel.js. Choose the approach that best fits your needs.
...
### Custom Integration
If you require kernel.js functionality not yet supported natively by Portal, a custom integration using Portal's EOA as a signer might be preferable.
...
Integrating kernel.js with Portal is straightforward once your application is set up. Portal provides an EOA wallet to use as a signer with Kernel.
...
const smartAccountSigner = providerToSmartAccountSigner(portalProvider);
