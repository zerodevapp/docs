...
### Custom Integration
If you would like to use kernel.js directly with a Privy project, you can set up a custom integration using Privys's EOA as a signer.
...
Integrating kernel.js with Privy is straightforward once your application is set up. Privy provides an EOA wallet to use as a signer with Kernel.
...
The following example demonstrates the use of Privy's react auth SDK to get the embedded wallet and use it as a signer for Kernel.js.
...
const smartAccountSigner = providerToSmartAccountSigner(privyProvider);
