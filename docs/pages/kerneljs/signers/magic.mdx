...
To use Magic with kernel.js, first create an application that integrates with Magic.
...
## Integration
Integrating kernel.js with Magic is straightforward after setting up the Magic SDK. Magic provides an Externally Owned Account (EOA) wallet to use as a signer with Kernel.
...
const smartAccountSigner = providerToSmartAccountSigner(magicProvider);
