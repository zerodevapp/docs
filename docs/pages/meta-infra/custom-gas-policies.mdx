...
- **Timeout Settings**: The amount of time the policy engine will wait for a response from your webhook URL. It’s important to keep response times low to reduce latency for your end users.
...
With the `logicalOperator` set to `'or'`, the transaction will proceed if either ZeroDev’s policy checks or the webhook approves it.
