...
You can find the bundler and paymaster RPCs for a specific ZeroDev project on the [ZeroDev dashboard](https://dashboard.zerodev.app/).

:::info

While you can, in principle, use a paymaster from one provider with a bundler from another, in practice, different providers are sometimes incompatible, so we recommend that if you want to use a specific provider, you use it for both the bundler and the paymaster.

:::

## Configuring the RPC for a specific infra provider

You can append the following query params (e.g., `?param=value`) to the RPC:
