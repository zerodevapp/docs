...
## 3. Setup wagmi config and provider
Please follow wagmi's instruction to setup [wagmi config](https://wagmi.sh/react/getting-started)
...
function Providers({ children }: { children: React.ReactNode}) {
  ...
  return (
    // Ensure ZeroDevProvider is nested within wagmi's provider
