...
- When a user enables CAB for a Magic Account account, they sign [a session key](/magic-account/usage/automating-transactions) for a web service that will automatically move their CAB tokens into an escrow.  Therefore, any CAB tokens that a user receives on their account will be automatically locked in the escrow.
