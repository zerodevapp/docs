...
Magic Account enables chain abstraction by implementing a protocol known as "MagicSpend++," [which was originally proposed by Socket](https://ethresear.ch/t/magicspend-spend-now-debit-later/19678).  Here's a quick summary of how MagicSpend++ (as implemented in Magic Account) works:

- When a user enables CAB for a Magic Account account, they sign [a session key](/magic-account/usage/automating-transactions) for a watcher service that will automatically move their CAB tokens into an escrow.  Therefore, any CAB tokens that a user receives on their account will be automatically locked in the escrow.
