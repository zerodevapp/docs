...
Magic Account improves on a protocol known as "MagicSpend++," [which was originally proposed by Socket](https://ethresear.ch/t/magicspend-spend-now-debit-later/19678).  Here's a quick summary of how Magic Account works:

- When a user enables CAB for a Magic Account account, they sign [a session key](/magic-account/usage/automating-transactions) for a watcher service that will automatically wrap the underlying tokens (e.g. USDC) into CAB tokens.  Therefore, any underlying tokens that a user receives on their account will be automatically turned into CAB tokens.
...
Here are five reasons why chain abstraction, and specifically Magic Account, is better than bridges:

### A single unified balance

With Magic Account, your user owns not Arbitrum USDC vs Base USDC vs mainnet USDC -- they just own USDC.  The USDC is one *chain-abstracted balance* that can be spent on any chain, and the user can deposit into this USDC balance from any chain as well.  This is a vastly simpler mental model for users, as they don't have to manually track and manage their liquidity across chains.

### No need for native gas tokens

Since Magic Account is built on ERC-4337, it’s able to use the “paymasters” mechanism to achieve gas abstraction — the user is able to pay gas with chain-abstracted tokens (e.g. USDC), or let the DApp sponsor gas for them.
...
With Magic Account, your user only has to wait for the target chain latency -- the latency for executing the user transaction.  In other words, a Magic Account transaction is just as fast as a regular transaction.

This is possible because, as aforementioned, Magic Account users don't interact with bridges directly.  Rather, solvers front liquidity for users, and it's the solvers that later go through bridges to get themselves repaid.
