...
## Add web variants of the native-only files
...
The web `useAuthenticateOAuth` hook runs a popup instead of a deep link. It returns to the same page that started auth, so it takes no arguments and does not need a dedicated `/oauth-callback` route on web:
...
Web export uses the `useExportWallet` / `useExportPrivateKey` hooks (in place of the native [`ZeroDevExportWebView`](/wallets/react-native/export-wallet)). They render the Turnkey iframe into a DOM node, which a `<View nativeID>` becomes under react-native-web:
...
## Route only the callbacks that still matter on web
