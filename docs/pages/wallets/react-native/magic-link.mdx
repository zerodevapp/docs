...
The flow: send → the backend emails the dashboard-configured redirect URL with `?code=<otp>` → the user taps it → the app opens at `/verify-email` → the route pairs the `code` from the URL with the persisted `otpId`/`otpEncryptionTargetBundle` and verifies.
  ...
  return (
    <>
      ...
      <Button
        ...
        onPress={() =>
          send.mutate({ email }, { onSuccess: savePendingMagicLink })
        }
      />
    </>
  );
...
- The Magic Link Redirect URL configured in the dashboard must match the intent-filter host and route path exactly; the emailed link adds the `code` query parameter.
