...
For full hook options and return values, see [`useRefreshSession`](/wallets/hooks/use-refresh-session).
...
Custom storage is also required in environments that do not provide browser `localStorage`, such as React Native  — see [React Native Configuration](/wallets/react-native/configuration) for the storage adapters the SDK ships for Expo.
