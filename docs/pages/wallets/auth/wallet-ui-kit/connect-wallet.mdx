...
:::info
With `magicLink`, the email link returns the user to your app with a verification code in the URL. Mount `<ConnectWallet />` on the page that link opens — on mount it detects the code, shows the verifying screen, and completes the sign-in on its own:

```tsx
// The page your magic link redirects to
