...
Magic links need a route in your app that can receive the redirect and call `useVerifyMagicLink`. For local development, this might be:

```txt
http://localhost:3000/verify
```

Make sure the redirect origin is also added to the project's ACL allowlist in the ZeroDev dashboard. If your project uses custom email templates or custom link behavior, configure those settings in the dashboard before testing magic links.
...
1. `useSendMagicLink` sends an email with a link to your `redirectURL` and returns an `otpId` plus an `otpEncryptionTargetBundle`.
