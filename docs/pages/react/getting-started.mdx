...
ZeroDev's React SDK is called `@zerodev/waas`, which stands for Wallet-as-a-Service. This signals that the React SDK provides higher-level abstractions over the [low-level SDK](/).
...
In the typical React pattern, `@zerodev/waas` is set up via a provider. A typical setup may look like this:
...
The best way to learn how to use `@zerodev/waas` is to look at [the examples](https://github.com/zerodevapp/waas-examples) and browse the hooks in the sidebar.
