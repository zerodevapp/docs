...
The mutation function creates a session for the v2 kernel account.
...
`CreateBasicSessionData | undefined`
  ...
  - ##### smartAccount
    `0x${string}`: The address of the smart account.
  - ##### enableSignature
    `0x${string}`: The signature to enable the session.
  - ##### permissions
