...
- You need to replace the `BUNDLER_RPC` with an [actual bundler RPC](/sdk/infra/intro).
...
```
import { getKernelAddressFromECDSA } "@zerodev/ecdsa-validator"

const smartAccountAddress = getKernelAddressFromECDSA(publicClient, eoaAddress, index)
```
