...
In Viem, a [public client](https://viem.sh/docs/clients/public.html) is an interface to a JSON-RPC API such as Infura or Alchemy.  Most bundlers today implement the standard JSON-RPC in addition to bundler RPC, so you can also use your bundler RPC for the public client.  Though for maximal stability, we recommend using your own RPC (e.g. Infura/Alchemy).
...
const publicClient = createPublicClient({
  ...
  // We recommend using your own RPC provider for maximal stability, but you can
  // also use the bundler RPC from the ZeroDev dashboard.
