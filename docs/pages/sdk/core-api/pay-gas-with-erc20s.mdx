...
4. Choose how the token's exchange rate against the native gas token is sourced. Three price sources are available:
    - **CoinGecko** — The paymaster fetches the real-time price from CoinGecko each time a UserOp is submitted, so the rate stays current without any manual updates. Auto-selected when the token is supported by CoinGecko on the chosen chain. Not available on testnets, since CoinGecko prices reflect real market liquidity that testnets don't have — use a custom price feed or static rate when developing on a testnet.
