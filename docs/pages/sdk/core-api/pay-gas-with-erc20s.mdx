...
const kernelClient = await createEcdsaKernelAccountClient({
  ...
  paymaster: gasTokenAddresses[mainnet.id]['USDC'],
})
```

### Standard API

Follow the [standard API for creating accounts](http://localhost:5173/sdk/core-api/create-account#standard-api).  Then, at the last step when you construct the Kernel client, do this:
...
Use the `getERC20PaymasterApproveCall` to construct a call that approves the paymaster with the ERC20 tokens:
