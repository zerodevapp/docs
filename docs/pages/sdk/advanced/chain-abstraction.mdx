...
### Sending chain-abstracted transactions

In the previous example, you are explicitly specifying specific chains for your input tokens.  If you don't care where tokens are coming from, you can achieve true "chain abstraction" by simply not specifying any `inputTokens`, like this:
...
In this case, the SDK will automatically determine which chain(s) to pull input tokens from.  By default, the SDK pulls input tokens from chains with the highest balance, if you would like to use a different logic, get in touch.

### Getting Chain-abstracted Balances (CAB)

You can get the aggregated balance of any tokens across chains.  This is known as a "chain-abstracted balance" or CAB.
