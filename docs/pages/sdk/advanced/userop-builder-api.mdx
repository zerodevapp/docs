...
The UserOp Builder API is available only to users on the Enterprise plan.
...
- **[EIP-7702](https://eips.ethereum.org/EIPS/eip-7702)** — EOA to smart account delegation
...
All endpoints require authentication via API key headers. One of the following headers must be included:
...
The `userOpHash` in the response is what you need to sign with your account's private key.
...
Poll this endpoint until you get a successful response. The user operation may take a few seconds to be included in a block.
...
Pre-warms the kernel client cache for a project/chain. Optional, but recommended to reduce latency on the first request.
...
[EIP-7702](https://docs.zerodev.app/sdk/advanced/eip7702) allows EOAs to delegate to a smart contract temporarily. To use an EOA as a Kernel smart account:
