...
- Collecting signatures is completed asynchronously from various signers.
- You need to collect enough signatures to meet the threshold weight.
- All signatures must use the same UserOperation object.
...
To update the multisig configuration (e.g., adding or removing signers), use the update config functionality. This works the same way for both ECDSA and passkey signers:
