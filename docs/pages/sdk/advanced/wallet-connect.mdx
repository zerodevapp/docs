...
- `kernelProvider`: An optional pre-initialized kernel provider.
    - If you are using wagmi with the capabilities pattern (see [the capabilities quickstart](/smart-wallet/quickstart-capabilities) for more information), you can retrieve the kernelProvider from wagmi.
