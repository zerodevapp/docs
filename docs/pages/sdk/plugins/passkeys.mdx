...
We highly recommend that you check out [the passkey demo]() to understand how the end-user experience could look like with passkeys.
...
### Setting up your passkey server
...
Head to the [ZeroDev dashboard](https://dashboard.zerodev.app), select a project, and copy the passkey URL:
...
const passkeyValidator = await createPasskeyValidator(publicClient, {
  ...
  serverUrl: 'your passkey server URL',
})
```
...
Then, you can pass your server URL as `serverUrl`.
...
Alternatively, you can customize the URL paths as such:
