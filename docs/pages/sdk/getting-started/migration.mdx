...
- * The same RPC is usable for both bundler and paymaster.
...
Note that the last part `42161` is the chain ID. Therefore, if you'd like to use the RPC for different chains programmatically, you can do something like:
...
Of course, you can also copy the RPC for different chains from the dashboard.
...
2. **Ensure you have the `viem@^2.21.40` version**
...
Replace the `UserOperation` Types
...
The default gas prices might be too high, so it's recommended to use this function to estimate them.
