...
You will be using the following packages (note that you are installing the `alpha` version of the ZeroDev SDK):

```bash
npm i @zerodev/sdk@alpha @zerodev/ecdsa-validator viem
```
...
Use the bundler/paymaster RPCs from your project home.  The private key can be any test EOA key.
