...
With Yi, you can sponsor gas for your users so they don't have to acquire gas tokens before using your app.

Note that if you are already using [chain abstraction](/sdk/using-yi/chain-abstraction), gas sponsorship may not be necessary because your users can already pay gas with their chain-abstracted balance.
...
## Send transactions using `wallet_sendCalls` with the paymaster URL

### Wagmi

If you are using Wagmi, you can send sponsored transactions with `sendCalls()` or `writeContracts()`, by including the following capabilities:
