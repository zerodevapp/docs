...
The killer feature of Yi is chain abstraction.  When chain abstraction is enabled for a token, the token's balance turns into a "chain-abstracted balance" (CAB) that you can deposit into from any network, and that you can spend on any network.

For example, let's say you have enabled chain abstraction for USDC.  If you deposit 100 USDC to a Yi account on Base, then deposit 200 USDC to the account on Polygon, the account will now hold 300 "chain-abstracted USDC."  You can then spend this 300 USDC on any chain.
...
Currently Yi supports only **USDC** for chain abstraction, on the following networks:
...
When a user enables chain abstraction, they will need to sign a message.  Under the hood, this message creates a [session](/sdk/using-yi/automating-transactions) that will automatically turn tokens on all supported networks into chain-abstracted balances (CAB).
...
When you send a transaction, Yi automatically spends CAB if the transaction needs CAB.  For example, if you are buying an NFT on Arbitrum that costs 500 USDC, and you don't have USDC on Arbitrum, then Yi will spend 500 USDC worth of CAB on this transaction.

### Wagmi

To send a transaction that might use CAB, use [`sendCalls()`](https://wagmi.sh/react/api/hooks/useSendCalls) or [`writeContracts()`](https://wagmi.sh/react/api/hooks/useWriteContracts).  Here's an example with `writeContracts`:
...
You don't normally need to "withdraw" CAB per se.  If you want to move the tokens to another account, you can simply do a transfer by [spending CAB](/sdk/using-yi/chain-abstraction#spending-chain-abstraction-balances-cab).
