...
Our `KernelClient` implements the Viem WalletClient interface. Although it is not directly compatible with Ethers.js, we have developed an `EIP1193Provider` that accepts a `KernelClient` as a constructor parameter. This provider enables using `KernelClient` with Ethers.js, as `window.ethereum` is used with Ethers.js.
