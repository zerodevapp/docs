...
In this tutorial, you will learn how to automate transactions for your users using [session keys](/sdk/permissions/intro). This is useful when you want to send transactions for your users from your server, for instance.
...
From the agent's side, create an [ECDSA signer](/sdk/permissions/signers/ecdsa) as the session key:
...
Now, on the owner side, approve the session key with [policies](/sdk/permissions/intro#policies):
...
[Refer to this page.](/sdk/advanced/multi-chain-signing)
...
That is, instead of creating an ECDSA validator, pass the EOA (the 7702 account) as `eip7702Account` into `createKernelAccount`, and don't set a `sudo` validator.
