...
Here, the `signer` will be able to call `action` if all `policies` are met.
...
Policies are the conditions under which the keys are usable. ZeroDev provides the following policies:
...
If that's confusing, just forget about actions. Most commonly, you will only set up signers and policies, and the `action` will default to Kernel's `execute()` function, which is enough for most needs.
