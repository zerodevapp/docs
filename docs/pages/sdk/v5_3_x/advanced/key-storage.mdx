...
The remote signer feature is a paid add-on. Please [contact us](https://t.me/derek_chiang) before using this feature in production to avoid service disruptions.

:::

Sometimes you can manage session keys or even actual private keys for your users, but you may not want to store the keys in your database.

ZeroDev provides a key management API that lets you generate private keys and sign with them. The private key is never transmitted to you or stored on your server—the API executes the signing remotely.
...
`remoteSigner` is a [Viem account](https://viem.sh/docs/accounts/privateKey) that you can use for signing.
...
When getting an existing key, specify the key's “address”.
