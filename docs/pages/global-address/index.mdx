...
Some common use cases of global addresses are:
...
- **Cross-chain deposits**: you can create a global address for your user for a specific chain.  Your user can then send funds to that address on any chain, and they will receive funds on the destination chain.
  - This is especially powerful for **onboarding funds from CEXs and fiat onramps**, since most CEXs/onramps only support withdrawing funds to a few chains.
    - For example, if your app is on Base but your user wants to use funds from a CEX that can only withdraw to Ethereum, you can create a global address so that when the user withdraws funds to that address, they will receive funds on Base.
...
const { globalAddress } = await createGlobalAddress({
  ...
  slippage,
  config: {
    baseUrl: 'https://global-address-server.onrender.com',
  },
})
```
...
- `config` is other configs, notably the `baseUrl` that specifies the API server.  For now, use `https://global-address-server.onrender.com`.
